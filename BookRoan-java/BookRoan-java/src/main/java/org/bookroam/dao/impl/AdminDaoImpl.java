...
//				.selectList(ADMIN_NAMESPACE + "selectAdminByUsername", name);
		List<Object> objectsLists =this.getSqlSession().selectList(ADMIN_NAMESPACE + "selectAdminByUsername", name);
		ArrayList<Admin> admins = new ArrayList<>();
		for (Object objectsList : objectsLists){
			admins.add((Admin) objectsList);
		}
		if (admins.isEmpty()) {
			return null;
		} else {
	...
	public List<Admin> selectAdminByUsernameLike(String name) {
		...
		List<Object> objectsLists =this.getSqlSession().selectList(ADMIN_NAMESPACE + "selectAdminByUsername", queryMap);
		ArrayList<Admin> admins = new ArrayList<>();
		for (Object objectsList : objectsLists){
			admins.add((Admin) objectsList);
		}
...
//				.selectList(ADMIN_NAMESPACE + "selectAdminByUsernameLike", queryMap);
	...
	public List<AdminMessage> selectAdminMessageByTitleLike(String title) {
		...
		@SuppressWarnings("unchecked")
		List<Object> objectsLists =this.getSqlSession()
				.selectList(ADMIN_NAMESPACE + "selectAdminMessageByTitleLike", queryMap);
		ArrayList<AdminMessage> adminMessages = new ArrayList<>();
		for (Object objectsList : objectsLists){
			adminMessages.add((AdminMessage) objectsList);
		}
